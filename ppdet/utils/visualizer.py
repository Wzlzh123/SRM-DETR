...
def draw_bbox(image, im_id, catid2name, bboxes, threshold):
    ...
    for dt in np.array(bboxes):
        ...
        if catid not in catid2color:
            if catid == 0:
                catid2color[catid] = np.array([255,0,0])
            elif catid == 1:
                ...
                catid2color[catid] = np.array([0, 255, 255])
            elif catid == 6:
                catid2color[catid] = np.array([148, 87, 235])
            elif catid == 7:
                catid2color[catid] = np.array([255, 127, 80])
            elif catid == 8:
                catid2color[catid] = np.array([135, 206, 250])
        ...
        # tw, th = draw.textlength(text)
        xy = (0,0)
        from PIL import ImageFont
        font_path = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
        font = ImageFont.truetype(font_path, 16)
        _,_,tw,th = draw.textbbox(xy,text,font=font)
        draw.rectangle(
