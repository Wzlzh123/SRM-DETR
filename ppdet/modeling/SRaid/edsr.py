...
class ResBlock(nn.Layer):
    def __init__(self, conv, n_feat, kernel_size, bias=True, bn=False, act=nn.ReLU(), res_scale=1):
                ...
                try:
                    ...
                    if name.find('tail') == -1:
                        raise RuntimeError(f"While copying the parameter named {name}, "
                                           f"whose dimensions in the model are {own_state[name].shape} and "
                                           f"whose dimensions in the checkpoint are {param.shape}.") from e
            elif strict:
                if name.find('tail') == -1:
