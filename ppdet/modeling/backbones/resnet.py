...
__all__ = ['ResNet', 'Res5Head', 'Blocks', 'BasicBlock', 'BottleNeck', 'ResNet_2']
...
class BottleNeck1(nn.Layer):
    ...
    def __init__(self,
        ...
        self.use_pdconv_x = use_pdconv_x
        self.use_pdconv_y = use_pdconv_y
        ...
        # PdConv_x
        if self.use_pdconv_x:
            self.branch2b = PdConv(
                ch_in,
                width,
                7,
                1,
        ...
        # PdConv_y
        if self.use_pdconv_y:
            self.branch2b2 = PdConv(
                ch_in,
                width,
                7,
                1,
        ...
        # 关键：根据实际启用分支数动态计算 branch2c 输入通道
        in_c_branch2c = width
        if self.use_pdconv_x:
            in_c_branch2c += width
        if self.use_pdconv_y:
...
class ResNet_2(nn.Layer):
    ...
    def __init__(self,
        ...
        self._model_type = 'ResNet_2' if groups == 1 else 'ResNeXt'
        assert num_stages >= 1 and num_stages <= 4
        self.depth = depth
        self.variant = variant
        self.groups = groups
...
#————————————————————————————————————————————————————————————————————————
# PdConv相关代码
class PdConv(nn.Layer):
    def __init__(
        self,
        in_channels: int = 1,
        out_channels: int = 1,
        kernel_size: int = 9,   # kernel size lzh
