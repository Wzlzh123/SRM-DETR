...
def deformable_key_aware_attention_core_func(query,value,key, value_spatial_shapes,
                                   value_level_start_index, sampling_locations,
                                   attention_weights):
    """
    Args:
    ...
    split_shape = [h * w for h, w in value_spatial_shapes]
    value_list = value.split(split_shape, axis=1)
    key_list = key.split(split_shape, axis=1)
    sampling_grids = 2 * sampling_locations - 1
    sampling_value_list = []
    sampling_key_list = []
    for level, (h, w) in enumerate(value_spatial_shapes):
        ...
        value_l_ = value_list[level].flatten(2).transpose(
            [0, 2, 1]).reshape([bs * n_head, c, h, w])
        key_l_ = key_list[level].flatten(2).transpose(
            [0, 2, 1]).reshape([bs * n_head, c, h, w])
        # N_, Lq_, M_, P_, 2 -> N_, M_, Lq_, P_, 2 -> N_*M_, Lq_, P_, 2
        sampling_grid_l_ = sampling_grids[:, :, :, level].transpose(
            [0, 2, 1, 3, 4]).flatten(0, 1)
    ...
    # attention_weights = attention_weights.transpose([0, 2, 1, 3, 4]).reshape(
    #     [bs * n_head, 1, Len_q, n_levels * n_points])
    key = paddle.stack(sampling_key_list,axis=-2).flatten(-2)
    value = paddle.stack(sampling_value_list,axis=-2).flatten(-2)

    key = key.transpose([0,2,3,1]).flatten(0,1)

    N_,Lq,DD_ = query.shape
    query = query.reshape([N_,Lq,n_head,DD_ // n_head])
    query = query.transpose([0,2,1,3]).flatten(0,2)

    query = query.unsqueeze(-2)
    dk = query.shape[-1]

    attention_weights = paddle.matmul(query,key.transpose([0,2,1])) / math.sqrt(dk)
    attention_weights = F.softmax(attention_weights,-1)

    value = value.transpose([0,2,3,1]).flatten(0,1)
    output = paddle.matmul(attention_weights,value)
    output = output.squeeze(-2).reshape([N_,n_head,Lq,c]).transpose([0,2,1,3])
...
def get_contrastive_denoising_training_group(targets,
    ...
    if label_noise_ratio > 0:
        ...
        # half of bbox prob
        mask = paddle.rand(input_query_class.shape) < (label_noise_ratio * 0.5)
        chosen_idx = paddle.nonzero(mask.astype('float32') * pad_gt_mask).squeeze(-1)
        # randomly put a new one here
        new_label = paddle.randint_like(
            chosen_idx, 0, num_classes, dtype=input_query_class.dtype)
        input_query_class.scatter_(chosen_idx, new_label)
    ...
    if split == True:
        tgt_size = num_denoising//2 + num_queries//2
        attn_mask = paddle.ones([tgt_size, tgt_size]) < 0
        # match query cannot see the reconstruction
        attn_mask[num_denoising//2:, :num_denoising//2] = True
        # reconstruct cannot see each other
