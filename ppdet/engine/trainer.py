...
class Trainer(object):
    ...
    def load_weights(self, weights, ARSL_eval=False):
        if self.is_loaded_weights:
            return
        self.start_epoch = 0
        load_pretrain_weight(self.model, weights, ARSL_eval, mode = self.checkpoint_mode)
        ...
        if visualize:
            for outs in results:
                ...
                for i, im_id in enumerate(outs['im_id']):
                    ...
                    vis0 = np.array(vis_image)
                    ir0 = np.array(ir_image)
                    print("==== AFTER OPEN VIS ====")
                    print(vis0.shape, vis0.dtype, vis0.min(), vis0.max(), vis0.mean())
                    print("==== AFTER OPEN IR ====")
                    print(ir0.shape, ir0.dtype, ir0.min(), ir0.max(), ir0.mean())
                    ...
                    # start = end
                    # save image with detection
                    save_vis_name = self._get_save_vis_image_name(output_dir, vis_image_path)
                    save_ir_name = self._get_save_ir_image_name(output_dir, ir_image_path)

                    # 改成 bmp，避免 png 压缩编码导致崩溃
                    save_vis_name = os.path.splitext(save_vis_name)[0] + '.bmp'
                    save_ir_name = os.path.splitext(save_ir_name)[0] + '.bmp'
                    ...
                    vis_image.save(save_vis_name, format='BMP')
                    ir_image.save(save_ir_name, format='BMP')
